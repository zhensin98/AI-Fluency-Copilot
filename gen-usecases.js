...
var GENERIC_M365_HABITS = [
  { id:'MH1', entry:'M365 Copilot Chat', baseSaved:25,
  ...
  { id:'MH2', entry:'Outlook', baseSaved:25,
    ...
    guardrails:'Review drafts carefully before sending; remove any internal-only references' },
  { id:'MH3', entry:'Teams', baseSaved:25,
    ...
    guardrails:'Confirm agenda with all parties before distributing; verify meeting summaries are accurate' },
  { id:'MH4', entry:'M365 Copilot Chat', baseSaved:35,
    ...
    guardrails:'Verify document currency; do not surface confidential files outside their intended audience' },
  { id:'MH5', entry:'Word', baseSaved:35,
    ...
    guardrails:'Validate all data claims; get sign-off from relevant stakeholders before distribution' },
  { id:'MH6', entry:'Word', baseSaved:30,
    ...
    guardrails:'Cross-check AI summary against original; flag any discrepancies for human review' },
  { id:'MH7', entry:'Excel', baseSaved:30,
...
roles.forEach(function(role) {
  ...
  chatCandidates.forEach(function(uc) { uc.id = 'UC-' + String(chatNum++).padStart(3,'0'); });
  m365Candidates.forEach(function(uc) { uc.id = 'UC-' + String(m365Num++).padStart(3,'0'); });

  var specific = chatCandidates.filter(function(u){ return !u.isGeneric; }).length;
  var generic  = chatCandidates.filter(function(u){ return  u.isGeneric; }).length;
  console.log('\nRole: ' + role.role + ' | ' + specific + ' specific + ' + generic + ' generic = 7 chat');
  chatCandidates.forEach(function(uc) {
    console.log('  [' + uc.habitId + (uc.isGeneric?' GENERIC':' score:'+uc.relevanceScore) + '] ' + uc.name);
  });
